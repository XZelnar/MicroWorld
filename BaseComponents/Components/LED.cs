...
{
    class LED : Component, Properties.IDrawBorder, Properties.ILightEmitting
    {
        #region JointLocs
        ...
        public float GetBrightness(float x, float y)
        {
            ...
            if ((v = (Logics as Logics.LEDLogics).Brightness) > 0)
            {
                ...
                float t = (float)((1 - Math.Sqrt(dx * dx + dy * dy) / Luminosity) * (v / NeededVoltage));
                t = t < 0 ? 0 : t;
                return t;
            }
            return 0f;
        ...
        {
            (Graphics as Graphics.LEDGraphics).DrawAOE(renderer, opacityMultiplier);
        }
        ...
        {
            var g = Graphics as Graphics.LEDGraphics;
            if (g.wasAOEDrawn)
            {
                g.AOEOpacity += 0.05f;
            }
        ...
        {
            Components.Graphics.LEDGraphics.LoadContentStatic();
        }
        ...
        public override void Reset()
        {
            ...
            var l = Logics as Components.Logics.LEDLogics;
            l.Brightness = 0;
        }
